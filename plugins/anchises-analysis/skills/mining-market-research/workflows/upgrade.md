...
- A check-only request performs a fresh fixed-repository lookup through the
  ...
  Skip the adapter's cache-only command. Compare against native installed
  inventory, not just the session's bundled version; report loaded, installed
  and latest versions separately when they differ.
...
`update_state.py probe --force --platform <host>` and feed the SAME captured
