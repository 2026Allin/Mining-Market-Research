...
6. `get_available_dates` for actual exchange/trade_date coverage; discover
   physical SQL tables with `list_stock_tables` and columns with `get_table_schema`.
   Price history uses `stock_daily`; never synthesize exchange-day table names.
   Screen fields and SQL columns are separate schemas: discover each before use.
...
`get_available_dates`, `list_stock_tables`, `screen_stocks`, and `run_readonly_sql` also return a
...
`resolve_company_identity` searches dynamically discovered exchange
