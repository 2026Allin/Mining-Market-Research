...
Claude Chat override: use [the Chat adapter](hosts/claude-chat.md)
for the plugin check, not the native CLI adapter below. Keep the independent
service check and receipt fields, label the version “当前会话加载版本”, and do not
claim an installed version. Normal diagnostics use the six-hour session cache;
explicit refresh bypasses it. A failed plugin check remains unknown.
...
> 更新提示：Mining Market Research 插件 `<target_version>` 已发布（当前为 `<installed_version>`）。如需现在更新，请回复：“请为我安装 Mining Market Research 更新。”
