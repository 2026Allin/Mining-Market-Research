...
requests follow [update-notifications.md](update-notifications.md), using a
separate six-hour successful-check cache and no hooks. Do not check on an unrelated request. Plugin release discovery is
...
`plugin_update_check`. This legacy diagnostic cache is separate from automatic reminders.
A valid successful result is fresh for 1 hour. An
...
- `update_available`: finish the requested operational response and append the fixed notice
...
probe and TTLs defined above. A diagnostic explicitly reports `current` as `已是最新版`, reports
