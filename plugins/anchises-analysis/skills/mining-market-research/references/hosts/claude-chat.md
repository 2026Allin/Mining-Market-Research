...
2. Add ALL these options to every subsequent action:
   `--platform claude --surface claude-chat --context-file <returned-path>`.
   ...
   `python3 scripts/update_state.py check --platform claude --surface claude-chat --context-file <returned-path> --allow-network`.
...
Read the resulting session record for the latest release when composing this
