"""Load and validate the Mining Market Research MCP descriptor snapshot."""
...
def _validate_dynamic_stock_access_policy(
    ...
    if not expected_screen_inputs.issubset(screen_input):
        raise ContractError(
    ...
    for tool_name in ("screen_stocks", "run_readonly_sql"):
        ...
        if not expected_policy.issubset(policy_properties) or "eligible" in policy_properties:
            raise ContractError(
                f"{tool_name} must publish eligible_by_query and the dynamic "
                "export policy contract"
            )
        if not expected_policy.issubset(policy.get("required", [])):
