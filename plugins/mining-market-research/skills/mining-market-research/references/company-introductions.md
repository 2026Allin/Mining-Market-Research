...
For news evidence, use the retrieval component in
[News Analysis](../workflows/news-analysis.md): search the corpus before host
web supplementation, reuse resolved identities and the service check, and share
the five-article call budget across this request. Keep `primary_task` unchanged.
...
Standalone introductions do not authorize `get_company_report`; use Company
Report only when the user requests that deliverable.
