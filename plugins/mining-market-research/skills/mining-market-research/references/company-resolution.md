...
Before a company report, reuse a complete verified identity from current chat
context or a reliable news response's `companies` when the issuer and listing
are unambiguous. Do not re-resolve it just to satisfy a fixed call sequence.
If news names multiple companies/listings, use the user's target or ask one
focused question. If reliable company information is absent, use the resolver
below; never infer a ticker from the headline.
...
For stock data, explain the unresolved identity and use the service's actual
supported exchanges when describing coverage, not a hard-coded exchange list.
Do not imply structured coverage outside the markets returned by the service.
...
`get_company_report` (or explicit fresh preparation). The research response may use
