...
request whenever a Mining Market Research Skill is selected explicitly or
...
If one is exposed, recommend revoking or rotating it and explain that Mining
Market Research does not use chat-supplied credentials.
...
Follow the active MCP access policy: company reports require `stock.read` and
linked `news_codes` additionally require `news.read` in scope-based modes.
Do not turn a news-linked report into an unlinked request after permission or
association failure. Public noauth access does not require credential entry.
