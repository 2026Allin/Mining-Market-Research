...
Apply this contract to every Mining Market Research request, including requests that
...
Read only the adapter for the active host:
[Codex](hosts/codex.md) or
[Claude Code](hosts/claude-code.md), or
[Claude Chat](hosts/claude-chat.md). Chat uses session-only
release checking and manual updates, not the native CLI protocol.
Resolve logical tool names using the active connection and loaded schemas.
If a required tool is missing, disclose the affected capability; do not assume
that installation or the checked-in snapshot proves runtime availability.
...
- Match the language of the current request, falling back to the conversation
  language.
- Call the product, plugin, and bundled MCP service **Mining Market Research**.
...
- Keep web and Mining Market Research data provenance distinguishable, not in
  mandatory separate chapters. Fusion combines reasoning while retaining each
  source's dates and identity.
...
- Keep the bundled plugin policy separate from MCP service capabilities. The
  ...
  not provide.
- When any of the six Skills is selected for a substantive business request,
...
- For requested news evidence, use the shared
  [news retrieval component](news-retrieval.md) without reclassification.
- For analysis_mode=fusion, the owning news/market workflow loads
  [event-market-analysis.md](../workflows/event-market-analysis.md) and its required
  company-type, timing, metrics and macro references even when no other Skill
  was selected. Components return evidence, not separate answers; reuse state.
  Track `news_article_calls` across all components; never exceed five per request.
- Only `company-report` may call `prepare_company_report_generation`.
- Only `company-report` may call `get_company_report`. Default report selection
  is `auto`; explicit refresh uses `refresh`. MCP owns report validity. The
  six-hour plugin-version check is not a report cache or expiry policy.
- Existing reports are faithfully presented/translated with original dates;
  generated research and translations stay only in the current conversation.
...
[plugin-update.md](plugin-update.md) for explicit update operations, and
