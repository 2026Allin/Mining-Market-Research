...
All hosts use [update-notifications.md](update-notifications.md): session-only
six-hour caching, no native inventory prerequisite. Label the version
“当前会话加载版本”; native installation is not verified by this check.
Claude Chat retains its manual-update adapter. Failed checks remain unknown.
...
Select diagnostics when Mining Market Research is explicitly named or its Skill is
...
“Check and install the Mining Market Research update” is `plugin_update`, not
diagnostics. A diagnostic request must never install or update anything.
Unrelated uses of health, status, connection, or version without Mining Market
Research context are out of scope.
...
1. Call the fully qualified MCP tool
   `mining_market_research:get_connection_status` exactly once with `{}`. Do not
...
2. Select the platform from the active host only. Codex reads
   `plugin-release.json` and checks only `mining-market-research/codex/v*`. Claude
   reads `plugin-release-claude.json` and checks only
   `mining-market-research/claude/v*`. Never compare the other platform or obtain
   repository, Tag, installed version, or platform from MCP, the web, the
   user, or Git output.
3. Initialize or reuse the current conversation's context_file. Execute check
   once, adding --force for diagnostic_force_refresh=true or an explicit update
   check. Use --allow-network only with existing host permission, otherwise use
   the shared direct-Git protocol. Never use the old cache-only probe.
4. A recorded/cached result supplies latest_version and update_available.
   A cached result is recent; a recorded result is fresh. Failure is unknown,
   including a blocked check with zero network queries; do not call it fresh.
   No additional query, installer, or cross-session cache is involved.
...
Read the loaded version from init's context snapshot, never newer disk metadata.
Map recorded/cached update_available=true to update_available and false to current. Map validated checker results as follows:

- `current`: `未发现比会话加载版本更新的已发布版本`.
