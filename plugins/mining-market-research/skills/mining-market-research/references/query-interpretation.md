...
| `full_report` | View/translate a company research report, refresh it, or perform a deep structured study or due-diligence report |
...
- A mixed full-report and technical request remains `full_report` with an
  attached market-data modifier. Present the selected report first and keep its
...
## Fusion mode (not another primary task)

Set analysis_mode=fusion for event impact, market reception, why a stock moved,
unusual-volume explanations, whether a breakout has news support, or combined
recent developments and price interpretation. News-led impact stays primary_task=news;
price/volume-led explanation stays market_data. Existing full_report ownership
and faithful report presentation remain unchanged.
A known event + "what does this mean for the company/stock?" selects fusion;
"what does this term mean?" is explanatory, not a demand for market research.
Headlines, faithful translation, article summaries, plain price tables, screens
without causal questions and CSV-only requests remain standard mode.
"News only", "no web" and explicit scope limits override default components.

For fusion, the owning news/market workflow loads event-market-analysis.md,
company-type priorities, time/metric rules and real-time macro research rules.
No extra independent Skill invocation is required. Do not reject a coherent
event-and-price question as two competing deep deliverables.
A follow-up can select fusion even if the prior turn only browsed headlines.
...
- Hand `full_report` to
  [the Company Report workflow](../workflows/company-report.md). Default to
  `get_company_report(mode=auto)`; explicit latest/regeneration intent selects
  `mode=refresh`. For `generation_ready`, execute live research immediately
  without asking whether to generate it. For `report_available`, present the
  original date and faithful translation, then ask once whether to refresh.
- An unambiguous agreement to a pending company-report refresh belongs to
  `full_report`, not plugin installation. News-only impact questions remain
  `news`; requesting research on the linked company may enter `full_report`.
- Hand `news` to [the News Analysis workflow](../workflows/news-analysis.md).
- Hand `comparison` to
...
- If a requested stock-data exchange is unavailable, state the current structured
  coverage returned by `get_available_exchanges`.
...
- Treat a watchlist as one request. For cross-exchange named instruments use
  exact `instruments` exchange/ticker pairs rather than a cross-product.
...
- For previews, show the requested page; complete-history analysis authorizes
  necessary cursor continuation. Use server aggregates only for requested metrics.
- For CSV-only requests, use the original query ID after provisional eligibility;
  do not first read all pages. Fields improve performance, not export eligibility.
  Actual export results determine success.
...
- When rows are truncated, distinguish an actual server-calculated statistic
  from the rows read and the displayed preview. Follow the delivery modes and
  stop conditions in [market-data-policy.md](market-data-policy.md).
- When news context is requested, use the shared news workflow: search the
  corpus first, then supplement using host web research when needed and available.
