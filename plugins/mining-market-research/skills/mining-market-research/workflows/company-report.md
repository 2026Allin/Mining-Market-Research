...
Use `get_company_report` through Mining Market Research. MCP selects an existing
report or prepares live research; the Host presents/translates the former or
executes the latter in the current conversation. Never calculate report expiry.
...
Do not reclassify inside this Skill. Default to `mode=auto`; explicit requests
for latest or regenerated research use `mode=refresh`. A `generation_ready`
response authorizes immediate live research; do not ask whether to generate it.

## Establish access and identity

First apply the continuation/refresh gate in
[report-workflow.md](../references/report-workflow.md). If answering a follow-up
on the current report, do not enter a new identity or report retrieval sequence.
...
then reuse a verified current-context identity or unambiguous returned news
`companies` identity. Otherwise call `resolve_company_identity` with
...
`prepare_company_report_generation` or `get_company_report`.

Core boundaries:

- Use `auto` to let MCP select the report; never implement report-content caching.
- On `report_available`, preserve `report.generated_at` and faithfully present
  or translate the report without new facts. Ask once whether to refresh.
- On `generation_ready`, complete live research, not just prompt preparation.
...
For NEW research on a miner, developer, explorer, or royalty/streaming company, also read
...
For a mixed full-report plus quantitative request, finish the selected report
first. Then read
...
user's language. Existing reports preserve their original structure and risk
rating; only new research must use the fixed English section headings.
...
Call the product **Mining Market Research**. Treat the report as analytical
