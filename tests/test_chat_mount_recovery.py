...
class ChatMountRecoveryTest(unittest.TestCase):
    def test_reported_bad_arguments_are_rejected_before_network(self):
        ...
        cases = [
            (['--session-id', 'chat-20260911-gold'], 'invalid_session_id'),
            (['--session-id', valid], 'missing_loaded_release'),
            (['--session-id', valid, '--loaded-release', '0.6.0-dev.11 claude.20260911133138'], 'invalid_loaded_release'),
        ]
    ...
    def test_init_generates_and_reuses_exact_context(self):
        ...
        with tempfile.TemporaryDirectory() as temp, patch.object(updates.tempfile, 'gettempdir', return_value=temp):
            ...
            self.assertEqual(updates.read_chat_context(first['context_file']),
                             (first['session_id'], meta['version'] + '+' + meta['release_id']))
            ...
            with self.assertRaisesRegex(updates.InputError, 'context_unavailable'):
                updates.read_chat_context(first['context_file'])

    def test_flattened_core_mount_init_record_cached_without_plugin_root(self):
        source = ROOT / 'plugins/anchises-analysis/skills/mining-market-research'
        with tempfile.TemporaryDirectory() as temp:
            mount = Path(temp) / 'skills/plugins/anchises-analysis:mining-market-research'
            ...
            refs = 'a'*40 + '\trefs/heads/main\n' + 'a'*40 + '\trefs/tags/anchises-analysis/claude/v0.6.0-dev.10\n'
